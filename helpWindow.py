...
class Help:
    ...
    def start(self, ending=False):
        ...
        for a in hint_text: #nacteni textu napovedy a nastaveni jeho polohy v okne
            ...
            textRect.center = (self.display.get_width()//2, shift)
            hint.append([text, textRect])
        ...
        while self.running:
            ...
            button.Button(self.display, "Zpět", self.display.get_width()-(self.display.get_width()/20), self.display.get_height()//100, self.display.get_width()//7, self.display.get_height()//10, "white", aqua, self.back)
