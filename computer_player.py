...
class Computer():
    ...
    def make_copy(self):
        ...
        for x in range(self.field_size): 
            ...
            for a in range(self.field_size):
                helper.append(Atoms_copy([], self.atoms[x][a].text, self.atoms[x][a].color, x, a))
                help.append(self.atoms[x][a].text)
            ...
            for a in range(self.field_size):
                for heave in range(len(move)):
                    ...
                    if (current_x < self.field_size) and (current_x >= 0) and (current_a < self.field_size) and (current_a >= 0):
                        self.copy[x][a].addNeighbour(self.copy[current_x][current_a]) #pridani souseda k atomu
    ...
    def pick_atom(self):
        ...
        optimal = []
        danger_atoms = self.find_danger(1, "blue") #najde nebezpecne atomy soupere, tedy ty ktere mohou explodovat v nasledujicim kole
        ...
        if depth <= 0:
            ...
            if player:
                ...
                for atom1 in good_choice:
                    ...
                    self.minimax(good_choice, danger_atoms, copy_atoms, atom_numbers, maxi+self.extension, optim, depth-1, False, False)
                    atom_numbers = hook[:]
            else:
                ...
                for atom2 in danger_atoms:
                    ...
                    self.minimax(good_choice, danger_atoms, copy_atoms, atom_numbers, maxi-self.extension, optim, depth-1, True, False)
                    atom_numbers = helper[:]
