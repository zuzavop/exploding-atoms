                ...
                if event.type == pygame.QUIT:
                    ...
                    if self.end_game:  # po ukonceni hry inicializuje po kliknuti navrat do menu
                        ...
                        for x in range(len(self.atoms)):
                            for a in range(len(self.atoms[x])):
                                self.atoms[x][
                                    a].clicked()  # hledani zmacknuteho atomu a pripadna inicializace zvetseni poctu castic v atomu
            ...
            for a in range(self.field_size):
                for heave in range(len(move)):
                    ...
                    if (current_x < self.field_size) and (current_x >= 0) and (current_a < self.field_size) and (
                            current_a >= 0):
                        self.atoms[x][a].add_neighbour(self.atoms[current_x][current_a])  # pridani souseda k atomu
