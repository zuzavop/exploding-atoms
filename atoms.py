...
class Atom:
    ...
    def run(self, size, x, y):
        ...
        smallText = pygame.font.Font("freesansbold.ttf", size//2)
        textSurf = smallText.render(f" {self.text} ", True, colors[self.color])
    ...
    def clicked(self):
        ...
        if self.x+self.size > mouse[0] > self.x and self.y+self.size > mouse[1] > self.y:
            if not((self.counter.counter == 1 and self.color=='blue') or (self.counter.counter == 0 and self.color=='red')):
    ...
    def explode(self):
        ...
        if self.atoms["red"] < self.field_size*self.field_size and self.atoms["blue"] < self.field_size*self.field_size:
            self.text  = 0
